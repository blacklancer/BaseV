...
class Trainer(BaseTrainer):
    def train(self):
        ...
        for iteration in range(start_iter, int(self.args.num_classes / self.args.nb_cl)):
            ...
            b1_model, b2_model, ref_model, ref_b2_model, lambda_mult, cur_lambda, last_iter = self.init_current_phase_model(
                iteration, start_iter, b1_model, b2_model)
            ...
            if iteration == start_iter:
                ...
                X_train, map_Y_train, map_Y_valid_cumul, X_valid_ori, Y_valid_ori = \
                    self.init_current_phase_dataset(iteration, \
                                                    start_iter, last_iter, order, order_list, X_train_total,
                                                    Y_train_total, X_valid_total, Y_valid_total, \
            ...
            # Imprint weights
            if iteration > start_iter:
                b1_model = self.imprint_weights(b1_model, b2_model, iteration, is_start_iteration, X_train, map_Y_train,
                                                self.dictionary_size)

            # Update training and test dataloader
            trainloader, testloader = self.update_train_and_valid_loader(X_train, map_Y_train, X_valid_cumul,
                                                                         map_Y_valid_cumul, \
            ...
            if iteration == start_iter and self.args.resume_fg:
                ...
                if iteration > start_iter:
                    ...
                    balancedloader = self.gen_balanced_loader(X_train_total, Y_train_total, indices_train_10,
                                                              X_protoset, Y_protoset, order_list)

                    # Training the model for different baselines
                    if self.args.baseline == 'lucir':
                        b1_model, b2_model = incremental_train_and_eval_lucir(self.args, self.args.epochs,
                                                                              self.fusion_vars, \
                        ...
                        b1_model, b2_model = incremental_train_and_eval_icarl(self.args, self.args.epochs,
                                                                              self.fusion_vars, \
                                                                              self.ref_fusion_vars, b1_model, ref_model,
                                                                              b2_model, ref_b2_model, tg_optimizer,
                                                                              tg_lr_scheduler, \
                    ...
                    b1_model = incremental_train_and_eval_zeroth_phase(self.args, self.args.epochs, b1_model, \
                                                                       ...
                                                                       cur_lambda, self.args.dist, self.args.K,
                                                                       self.args.lw_mr)

                    # Select the exemplars according to the current model
            X_protoset_cumuls, Y_protoset_cumuls, class_means, alpha_dr_herding = self.set_exemplar_set(b1_model,
                                                                                                        b2_model, \
                                                                                                        is_start_iteration,
                                                                                                        iteration,
                                                                                                        last_iter,
            ...
            top1_acc_list_ori, top1_acc_list_cumul = self.compute_acc(class_means, order, order_list, b1_model,
                                                                      b2_model, X_protoset_cumuls, Y_protoset_cumuls, \
                                                                      X_valid_ori, Y_valid_ori, X_valid_cumul,
                                                                      Y_valid_cumul, iteration, is_start_iteration,
