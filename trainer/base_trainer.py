...
class BaseTrainer(object):
    ...
    named trainer.py in the same folder.
    """
    ...
    def set_dataset_variables(self):
        ...
        if self.args.dataset == 'cifar100':
            ...
            self.evalset = torchvision.datasets.CIFAR100(root='./data',
                                                         ...
                                                         download=False,
                                                         transform=self.transform_test)
            self.balancedset = torchvision.datasets.CIFAR100(root='./data',
                                                             train=False,
                                                             download=False,
                                                             transform=self.transform_train)
            # Set the network architecture
            # 注意这里知识确定针对数据集的网络种类
            # 实际上的初始化在def init_current_phase_model函数里面
            ...
            # Set the dictionary size
            # 实际上就是每个class的最大数量
            # 这里是使用原型列表存储图片，同时使用另一个herding列表存储下标
    ...
    # 用了
    # 这里实际上是用一个array把所有的image按照class进行了拆解
    # 然后用另一个herding去记录选择的下标
    # 这样难道不是全都存起来了吗？我他妈的有点裂开啊
    def init_prototypes(self, dictionary_size, order, X_train_total, Y_train_total):
        ...
        alpha_dr_herding = np.zeros((int(self.args.num_classes / self.args.nb_cl), dictionary_size, self.args.nb_cl),
                                    np.float32)
        if self.args.dataset == 'cifar100':
            # CIFAR-100, directly load the tensors for the training samples
            prototypes = np.zeros((self.args.num_classes, dictionary_size, X_train_total.shape[1],
    ...
    # 用了
    # 上面的init_prototype是针对整个大循环的，而这里是针对每个阶段的
        ...
        if self.args.dataset == 'cifar100':
            ...
            for cls_idx in range(iteration * self.args.nb_cl, (iteration + 1) * self.args.nb_cl):
                ...
                cls_features = compute_features(self.args, self.fusion_vars, b1_model, b2_model, \
                                                tg_feature_model, is_start_iteration, evalloader, num_samples,
                                                num_features)
    ...
    def set_optimizer(self, iteration, start_iter, b1_model, ref_model, b2_model, ref_b2_model):
        ...
        if iteration > start_iter:
            ...
            tg_optimizer = optim.SGD(tg_params_new, lr=self.args.base_lr2, momentum=self.args.custom_momentum,
            ...
            if self.args.branch_mode == 'dual':
                ...
                fusion_optimizer = optim.SGD(self.fusion_vars, lr=self.args.fusion_lr,
                ...
                fusion_optimizer = optim.SGD(self.fusion_vars, lr=0.0, momentum=self.args.custom_momentum,
                                             weight_decay=self.args.custom_weight_decay)
            else:
            ...
            b1_model = b1_model.to(self.device)
            tg_optimizer = optim.SGD(tg_params, lr=self.args.base_lr1, momentum=self.args.custom_momentum,
                                     weight_decay=self.args.custom_weight_decay)
            if self.args.branch_mode == 'dual':
                fusion_optimizer = optim.SGD(self.fusion_vars, lr=self.args.fusion_lr,
                                             momentum=self.args.custom_momentum,
    ...
    def compute_acc(self, class_means, order, order_list, b1_model, b2_model, X_protoset_cumuls, Y_protoset_cumuls, \
                    X_valid_ori, Y_valid_ori, X_valid_cumul, Y_valid_cumul, iteration, is_start_iteration,
    ...
    def set_exemplar_set(self, b1_model, b2_model, is_start_iteration, iteration, last_iter, order, alpha_dr_herding,
                         prototypes):
